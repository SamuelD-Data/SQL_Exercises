...
SELECT SUM(DUPLICATE_COUNTS.USER_COUNT) AS USERS_WITH_DUPLICATE_USERNAMES, COUNT(DUPLICATE_COUNTS.USER_COUNT) AS TOTAL_DUPLICATE_USERNAMES
FROM
(SELECT LOWER(CONCAT(SUBSTR(FIRST_NAME,1,1), 
					SUBSTR(LAST_NAME,1,4), 
					'_', 
					...
					SUBSTR(BIRTH_DATE,3,2)))
					AS USERNAME, 
		COUNT(EMP_NO) AS USER_COUNT
FROM employees
GROUP BY USERNAME
HAVING USER_COUNT > 1) AS DUPLICATE_COUNTS;

-- 13251 DUPLICATE USERNAMES -- 
-- 27403 USERS WITH DUPLICATE USERNAMES --